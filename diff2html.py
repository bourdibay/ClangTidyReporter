...
<head>
    <meta charset="{1}" />
    <meta name="generator" content="diff2html.py" />
...
def linediff(s, t):
    ...
    '''
    if len(s):
        s = reduce(lambda x, y:x+y, [ sane(c) for c in s ])
    if len(t):
        t = reduce(lambda x, y:x+y, [ sane(c) for c in t ])
...
def convert(s, linesize=0, ponct=0):
    ...
    for c in s:
        ...
        # special html chars
        elif ord(c) in html.entities.codepoint2name.keys():
...
def parse_input(input_file, output_file, input_file_name, output_file_name,
    ...
    if not exclude_headers:
        ...
        tt = html_hdr.format(title_suffix, encoding, desc, "", modified_date, lang)
